...
do_join(State, Ref, ChatName) ->
    SPID = whereis(server),
	GPID = whereis(list_to_atom(State#cl_st.gui)),
	case maps:is_key(ChatName, State#cl_st.con_ch) of
		true ->
			GPID!{result,self(),Ref,err},
			{ok,State};
		false ->
			SPID!{self(),Ref,join,ChatName},
			receive
				{From,Ref,connect,History} ->
					GPID!{result,self(),Ref,History},
					{ok,State#cl_st{
						gui = State#cl_st.gui,
...
do_leave(State, Ref, ChatName) ->
	...
	case maps:is_key(ChatName, State#cl_st.con_ch) of
		false ->
			...
			{ok,State};
		true ->
			SPID!{self(),Ref,leave,ChatName},
			receive
				{_,Ref,ack_leave} ->
					GPID!{result,self(),Ref,ok},
					{ok,State#cl_st{
						gui = State#cl_st.gui,
						nick = State#cl_st.nick,
...
do_new_nick(State, Ref, NewNick) ->
    ...
    case NewNick /= State#cl_st.nick of
		false ->
			GPID!{result,self(),Ref,err_same},
			{ok,State};
		true ->
			SPID!{self(),Ref,nick,NewNick},
			receive
				{SPID,Ref,ok_nick} ->
					GPID!{result,self(),Ref,ok_nick},
					{ok,State#cl_st{
						gui = State#cl_st.gui,
						nick = NewNick,
						con_ch = State#cl_st.con_ch
					}};
				{SPID,Ref,err_nick_used} ->
					GPID!{result,self(),Ref,err_nick_used},
					{ok,State}
			end
	end.
...
do_msg_send(State, Ref, ChatName, Message) ->
    GPID = whereis(list_to_atom(State#cl_st.gui)),
	RPID = case maps:find(ChatName, State#cl_st.con_ch) of
		{ok,Room} -> Room
	end,
	RPID!{self(),Ref,message,Message},
	receive
		{_,Ref,ack_msg} ->
			GPID!{result,self(),Ref,{msg_sent,State#cl_st.nick}},
			{ok,State}
	end.
